...
import com.xqx.ych.common.protocol.mq.PaidMQ;
...
public class PayProcessor implements IMessageProcessor {
    ...
    public boolean handleMesage(MessageExt messageExt) {
        try {
            ...
            final PaidMQ paidMQ = JSON.parseObject(body, PaidMQ.class);
            //日志打印，方便调试----打印消息内容打印id  keys body
            log.info("pay PayProcessor recive message:" + messageExt);

            Enroll record = new Enroll();
            record.setEnrollId(paidMQ.getEnrollId());
            record.setEnrollStatus(paidMQ.getEnrollStatus());
            enrollMapper.updateByPrimaryKeySelective(record);
